...
def add_post(request):
    if request.method == 'POST': # user post request koreche 
        post_form = forms.PostForm(request.POST,request.FILES) # user er post request data ekhane capture korlam 
        if post_form.is_valid(): # post kora data gula amra valid kina check kortechi
            ...
            return redirect('add_post') # shob thik thakle take add author ei url e pathiye dibo
    else: # user normally  website e gele blank from pabe
...
def edit_post(request,id):
    ...
    if request.method == 'POST': # user post request koreche 
        ...
        if post_form.is_valid(): # post kora data gula amra valid kina check kortechi
            post_form.instance.author = request.user
            post_form.save() # jodi data valid hoy taile database e save korbo
            return redirect('homepage') # shob thik thakle take add author ei url e pathiye dibo
    return render(request,'add_post.html',{'form':post_form})
